...
def interactive_camera_placement(pos_scale=1.,
    ...
    if show_plot:
        ...
        # 第二个窗口
        plt.figure(2)
        plt_im2 = plt.imshow(np.zeros((240,320,4)))
        plt.axis('off')
        plt.tight_layout(pad=0)
    ...
    while 1:
        ...
        view_mtx = pb.computeViewMatrixFromYawPitchRoll(target_pos,
                                                        ...
                                                        upAxisIndex)
        # 第二个视角
        view_mtx2 = pb.computeViewMatrixFromYawPitchRoll(target_pos,
                                                        values['distance']/2,
                                                        values['yaw'],
                                                        values['pitch'],
                                                        values['roll'],
        ...
        #update visual aids for camera, target pos (useless , only for debug)
        ###
        ...
        img2 = pb.getCameraImage(width, height, view_mtx2, proj_mtx)[2]
        ...
        if show_plot:
            ...
            plt.figure(2)
            plt_im2.set_array(img2)
            plt.gca().set_aspect(height/width)
            plt.draw()
            plt.pause(0.1)

        if old_print_val != values['print']:
            print(old_print_val, values['print'])
